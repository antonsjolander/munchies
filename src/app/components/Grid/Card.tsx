...
import { Tag } from "../Tag";
import { BASE_URL } from "@/app/lib/contants";
...
export async function Card({
	...
	try {
		const data = await fetch(`${BASE_URL}/open/${id}`);
		...
		return (
			<Link
				...
				} group flex flex-col justify-between focus-visible:outline-green relative overflow-hidden bg-white flex-shrink-0 p-4 border rounded-lg border-stroke hover:border-green transition-colors duration-150 ease-out`}
			>
		...
		console.error(error);
		return (
			<div className="aspect-[327/202] text-body">
				Couldn´t fetch resturant card
			</div>
		);
	}
}
